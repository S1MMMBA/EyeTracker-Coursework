...
2. СПЕКТРАЛЬНЫЕ КОЭФФИЦИЕНТЫ (частотная область, FFT)
   ...
   - Фазы FFT-бинов крайне чувствительны к субмиллисекундным сдвигам начала
     захвата — надёжный источник хаоса
...
5. ВРЕМЕННА́Я НЕСТАБИЛЬНОСТЬ (jitter)
   - Δt между последовательными пиками давления — вибрато, дрожание голоса
   - Физиологически непредсказуем; период голосовых связок варьируется
     даже при попытке держать тон

СХЕМА ОБЪЕДИНЕНИЯ ГОЛОСОВОЙ И ГЛАЗНОЙ ЭНТРОПИИ
===============================================

Используется ХЭШ-МИКШИРОВАНИЕ (hash mixing):

    combined = SHA3-256(eye_bytes || voice_bytes || timestamp)

Это стандартная техника смешения источников энтропии (аналог /dev/random в Linux):
- Если один источник скомпрометирован — комбинация всё равно непредсказуема
- Порядок конкатенации фиксирован → детерминированный вывод при одних входах
- SHA3-256 обладает эффектом лавины: 1 бит изменения → ~128 бит изменений в хэше

Дополнительно применяется XOR-свёртка для увеличения диффузии:
    pool[i] ^= voice_bytes[i % len(voice_bytes)]
"""
...
# MAX_POOL      = 2000       # Максимальный размер пула энтропии (байт)
...
class VoiceEntropyCollector:
    ...
    def _process_frame(self, frame: np.ndarray):
        ...
        with self._lock:
            ...
            # if len(self._entropy_pool) > MAX_POOL:
            #     del self._entropy_pool[:len(self._entropy_pool) - MAX_POOL]

        # ── Оценка качества энтропии (энтропия Шеннона) ───────────────────
        self._update_entropy_quality()

    def _update_entropy_quality(self):
        """Приближённая оценка качества энтропии по распределению байт."""
        with self._lock:
            pool = bytes(self._entropy_pool[-256:])      # последние 256 байт
        ...
        mask   = probs > 0
        H      = -np.sum(probs[mask] * np.log2(probs[mask]))  # 0–8 бит/байт
        self._entropy_quality = H / 8.0                        # нормализованное 0–1
...
def combine_entropy(eye_bytes: bytes, voice_bytes: bytes) -> bytes:
    ...
    Никакого хэширования — только структурное объединение сырых данных.
    Хэширование и сжатие делегированы в derive_key / generate_combined_key.

    Алгоритм:

      Шаг 1 — Конкатенация: eye_bytes || voice_bytes
        Сохраняем все биты обоих источников в оригинальном виде.
        Порядок фиксирован: сначала взгляд, потом голос.

      Шаг 2 — XOR-наложение поверх конкатенации.
        Берём более короткий источник и XOR-им его поверх соответствующего
        участка пула, не затирая и не дублируя ни один байт.
        Позиции, где оба источника перекрываются, несут влияние обоих.
        Позиции только одного источника остаются нетронутыми.

    Возвращает bytes длиной len(eye) + len(voice).
    Все накопленные биты присутствуют — ничего не обрезается и не сжимается.
    """
    ...
    # ── Шаг 1: конкатенация всех сырых байт ──────────────────────────────
    # pool = [eye_bytes | voice_bytes]
    pool = bytearray(eye_bytes) + bytearray(voice_bytes)

    # ── Шаг 2: XOR-наложение короткого источника на начало пула ──────────
    #
    # Если voice короче — XOR накладывается на первые voice_len байт пула
    # (то есть на начало eye-региона), внося влияние голоса в глазные данные.
    #
    # Если eye короче — XOR накладывается на первые eye_len байт voice-региона
    # (смещение eye_len), внося влияние взгляда в голосовые данные.
    #
    # В обоих случаях каждый байт короткого источника изменяет ровно одну
    # позицию пула. Длина пула не меняется.
    #
    if voice_len <= eye_len:
        for i in range(voice_len):
            pool[i] ^= voice_bytes[i]
    else:
        for i in range(eye_len):
            pool[eye_len + i] ^= eye_bytes[i]

    print(
        f"  combine_entropy: eye={eye_len}B  voice={voice_len}B  "
        f"pool={len(pool)}B  (без хэширования)"
    )
